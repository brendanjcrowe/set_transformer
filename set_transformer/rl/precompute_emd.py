...
Batch 7.5 of the harness centralisation (``refactor_plans.md`` section 7, 2026-09-13): the body
of ``experiments/ant_tag/2b_precompute_emd.py`` moved here unchanged; that script is an entry
point of this module, and the package spelling is::

    python3 -m set_transformer.rl.precompute_emd --data_path <dataset.npz> --sinkhorn_blur 0.01
    python3 -m set_transformer.rl.precompute_emd --domain ant_tag --variant smart --sinkhorn_blur 0.01

(the second form takes the variant's collected dataset under the run root, decision 1).

Env-generic: reads the ``.npz`` the collector wrote (Ant-Tag or Odd-Even), loads it EXACTLY as
the reconstruction pretraining will -- same ``(x - particle_centre) / particle_scale`` mapping,
same weights -- and writes the pairwise debiased-Sinkhorn matrix over the resulting clouds plus
a JSON sidecar recording what it was built from. Only the ALIGNED pretraining arm
(``--align_lambda > 0``) needs this step.

Weights. When the dataset carries PF weights the matrix is between WEIGHTED measures (mass in
the measure, never in the metric), with each cloud's debiasing self-term computed under its own
weights. This is what makes the matrix informative on the Odd-Even exact-support filter, where
every cloud has the same 50-state support and only the weights differ -- an unweighted matrix
there is identically zero, and the command refuses to write one unless told to.
``--ignore_weights`` gives the unweighted ablation and must then also be passed to pretraining.

Blur and scaling MUST equal the ``--sinkhorn_blur`` / ``--sinkhorn_scaling`` you will train
with: the alignment target and the reconstruction objective have to measure the same geometry.
Pretraining reads the sidecar and refuses a mismatch, as it refuses a matrix built from a
different dataset file (``data_sha256``), a different frame (``particle_scale`` /
``particle_centre``) or a different weightedness.

Cost is O(N^2) Sinkhorn calls and 4 N^2 bytes: about 1 h and 1.6 GB for 20k clouds on an RTX
4070 Ti. ``--max_samples K`` builds the matrix over the first K rows only; pretraining then cuts
the dataset to those same rows (the sidecar records ``n_samples``). Resumable at row-block
granularity. The matrix lands beside the dataset as ``<stem>_emd.npy`` (+ ``.json`` sidecar).
"""
...
def build_parser(prog: str | None = None) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--data_path", type=str, default=None,
                        help="The .npz from the collector (any env). Default: the variant's "
                             "collected dataset under the run root (--domain --variant).")
    parser.add_argument("--domain", choices=sorted(_domains.DOMAIN_NAMES), default=None,
                        help="With --variant: locate the dataset under the run root when "
                             "--data_path is not given.")
    parser.add_argument("--variant", type=str, default=None)
    parser.add_argument("--output_root", type=str, default=None,
                        help="Root of the shared run layout for the --domain/--variant lookup.")
    ...
    parser.add_argument("--sinkhorn_blur", type=float, default=DEFAULT_BLUR,
                        help="Sinkhorn eps in NORMALIZED coordinate units; must "
                             "equal the pretraining's --sinkhorn_blur.")
    parser.add_argument("--sinkhorn_scaling", type=float, default=DEFAULT_SCALING,
                        help="Epsilon-scaling decay; must equal the pretraining's "
                             "--sinkhorn_scaling.")
...
def main(argv: Sequence[str] | None = None, *, prog: str | None = None) -> Path:
    ...
    if args.data_path is None:
        if not (args.domain and args.variant):
            parser.error("--data_path not given: pass it, or --domain and --variant to take the "
                         "variant's collected dataset under the run root")
        candidate = run_records.dataset_path(args.domain, args.variant, root=args.output_root)
        if not candidate.exists():
            parser.error(f"no collected dataset at {candidate}; collect one or pass --data_path")
    ...
    extra = {
        ...
        # 7.5: the same provenance the run records carry.
        "command": " ".join([prog or "set_transformer.rl.precompute_emd", *argv]),
    ...
    print("Next: python3 -m set_transformer.rl.pretrain --data_path {} --emd_matrix_path {} "
          "--align_lambda 0.2 --sinkhorn_blur {} --sinkhorn_scaling {}{}".format(
