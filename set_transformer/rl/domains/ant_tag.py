"""Ant-Tag domain pieces for the RL harness: reward shaping, curriculum, particle-filter glue.

Moved here 2026-09-12 from ``experiments/ant_tag/4_train_rl_frozen.py`` (change 1a of the
harness centralisation, ``refactor_plans.md`` in the parent repo). The bodies are unchanged;
that file now re-exports these names, so the numbered scripts, eval scripts, diagnostics and
tests that import it by flat name keep getting the same objects.
...
This module imports no env package and sets no matplotlib backend: it is importable without
MuJoCo. Registering the envs (``import pdomains``) is the caller's job.
"""
