"""
Evaluate a weighted-Gaussian checkpoint (4_train_rl_gaussian.py) on the REAL sparse tag reward.
...
``4_train_rl_gaussian`` below is what lets SB3 unpickle ``WeightedGaussianFeaturesExtractor`` out of a saved
...
Usage:
    python3 eval_scripts/eval_true_reward_gaussian.py --variant smart \\
        --model_path runs/ant_tag_gaussian_smart/<run>/models/gaussian_agent.zip \\
...
importlib.import_module("4_train_rl_gaussian")  # noqa: F401
...
def main(argv=None):
    ...
    return _shared_main(argv, domain="ant_tag", prog="eval_true_reward_gaussian.py")
