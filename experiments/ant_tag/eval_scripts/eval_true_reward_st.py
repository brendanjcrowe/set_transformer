"""
Evaluate a Set Transformer checkpoint (4_train_rl_st.py) on the REAL sparse tag reward.
...
``4_train_rl_st`` below is what lets SB3 unpickle ``SetTransformerFeaturesExtractor`` out of a saved
...
Usage:
    python3 eval_scripts/eval_true_reward_st.py --variant smart \\
        --model_path runs/ant_tag_st_smart/<run>/models/st_agent.zip \\
        --vecnormalize_path runs/ant_tag_st_smart/<run>/models/vecnormalize.pkl \\
        --n_episodes 100
"""
import importlib
import sys
...
importlib.import_module("4_train_rl_st")  # noqa: F401
...
def main(argv=None):
    ...
    return _shared_main(argv, domain="ant_tag", prog="eval_true_reward_st.py")
