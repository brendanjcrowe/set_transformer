"""
Evaluate a weighted-CGF checkpoint (4_train_rl_cgf.py) on the REAL sparse tag reward.
...
``4_train_rl_cgf`` below is what lets SB3 unpickle ``WeightedCGFFeaturesExtractor`` out of a saved
``policy_kwargs`` when the zip names that module.

Usage:
    python3 eval_scripts/eval_true_reward_cgf.py --variant smart \\
...
importlib.import_module("4_train_rl_cgf")  # noqa: F401
...
def main(argv=None):
    ...
    return _shared_main(argv, domain="ant_tag", prog="eval_true_reward_cgf.py")
