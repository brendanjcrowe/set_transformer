"""
Evaluate a pointnet checkpoint (4_train_rl_pool.py) on the REAL sparse tag reward.
...
``4_train_rl_pool`` below is what lets SB3 unpickle ``PointNetFeaturesExtractor`` out of a saved
...
Usage:
    python3 eval_scripts/eval_true_reward_pointnet.py --variant smart \\
        --model_path runs/ant_tag_pointnet_smart/<run>/models/pointnet_agent.zip \\
        --vecnormalize_path runs/ant_tag_pointnet_smart/<run>/models/vecnormalize.pkl \\
        --n_episodes 100
"""
import importlib
import sys
...
def main(argv=None):
    ...
    return _shared_main(argv, domain="ant_tag", prog="eval_true_reward_pointnet.py")
