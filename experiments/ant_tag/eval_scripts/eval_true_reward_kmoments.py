"""
Evaluate a kmoments checkpoint (4_train_rl_pool.py) on the REAL sparse tag reward.
...
``4_train_rl_pool`` below is what lets SB3 unpickle ``WeightedKMomentsFeaturesExtractor`` out of a saved
...
Usage:
    python3 eval_scripts/eval_true_reward_kmoments.py --variant smart \\
        --model_path runs/ant_tag_kmoments_smart/<run>/models/kmoments_agent.zip \\
        --vecnormalize_path runs/ant_tag_kmoments_smart/<run>/models/vecnormalize.pkl \\
        --n_episodes 100
"""
import importlib
import sys
...
def main(argv=None):
    ...
    return _shared_main(argv, domain="ant_tag", prog="eval_true_reward_kmoments.py")
