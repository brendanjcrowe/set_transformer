"""
Shared Ant-Tag RL pieces: reward shaping, visibility / evasion curriculum, PF glue.

STATUS (2026-09-11): this file is a LIBRARY, not a runnable arm. Its runnable part -- the
May-2026 "pretrained Set Transformer as a fixed feature processor inside the env" path
(AntTagPretrainedProcessor, make_ant_tag_pretrained_env, train_ant_tag_pretrained) -- was
moved to to_be_deleted/ant_tag_4_train_rl_frozen_legacy_main.py. It ran the encoder inside
the env wrapper, so the encoder could never be trained or finetuned; the live arms put the
encoder inside the SB3 policy (4_train_rl_st.py --pretrained_st_model_path [--st_frozen]).

What stays, because 4_train_rl_cgf.py (and through it the ST / Gaussian / pool arms, the
eval scripts and diagnostics) imports it from here:
  PFRewardShapingWrapper, CurriculumVisibilityWrapper, CurriculumCallback, _CurriculumRouter,
  get_ant_tag_pf_kwargs, ant_tag_pf_interaction_mapper (+ the _set_*_recursive helpers).
tests/test_ant_tag_shared_pieces_regression.py pins that surface. Moving these into the
package (set_transformer/rl/) is the next cleanup step; the file name is historical.
"""
...
if __name__ == "__main__":
    raise SystemExit(
        "4_train_rl_frozen.py is a library of shared Ant-Tag wrappers, not a runnable arm "
        "(since 2026-09-11). For a frozen pretrained Set Transformer use:\n"
