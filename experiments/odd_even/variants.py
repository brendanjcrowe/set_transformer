"""Registry of Odd-Even POMDP variants: which particle filter goes with which env.

The Ant-Tag mirror of this file (experiments/ant_tag/variants.py) replaced 19
one-line wrapper files, and the reasoning transfers unchanged: every script in
this directory needs the same facts about a variant -- the gym env id, the
matching particle filter, the state range and where its runs live -- and a
fact copied into seven scripts is a fact that can disagree with itself.

Two things this makes structural rather than remembered:

1. **The env/filter pairing.** The filter's likelihood table MIRRORS the env's
   own observation model (the Gaussian evaluated on the candidate's parity set
   and renormalized over just that set). If the two drift apart, belief
   propagation diverges from the env silently -- the weights are simply wrong
   and nothing raises. `n_dist_size` travels with the pair for the same
   reason: the filter builds its table from it, so a value that disagrees with
   the env's registration produces a filter for a different POMDP.

2. **The episode cap.** Gymnasium already knows it, so `episode_cap()` READS
   `max_episode_steps` off the registration rather than defaulting. On this
   domain a wrong cap does not merely mis-measure: the transient (steps 1-21,
   where the belief is still sharpening) is 42% of a 50-step episode and 10%
   of a 200-step one, so a pooled mean taken at the wrong cap is not
   comparable with anything.

What is deliberately ABSENT, compared with the Ant-Tag registry: there is no
visibility curriculum, no evasion schedule and no EVADING set. The hidden
state here is static and the action is a prediction, so nothing the agent does
changes the state or the observation stream. There is no schedule to ramp.

Adding a variant: register the env id in pdomains, then add one entry here,
and it appears in every script at once.
"""
...
# Package root on sys.path so `set_transformer` resolves to the package and
# not to the submodule directory of the same name (PITFALLS.md section 7).
# parents[2] is correct at experiments/<domain>/; a script one level deeper
# (eval_scripts/) must use parents[3].
