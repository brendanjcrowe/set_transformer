"""
Generate a dataset from the Ant-Heaven-Hell POMDP environment with
particle filtering.

This script creates a dataset from the Ant-Heaven-Hell environment, using a
...
Usage:
    python particle_filter_ant_heaven_hell.py --num_trajectories 50 \\
        --timesteps 100 --output_file ant_heaven_hell_pf_dataset.npy
"""
...
from environments.ant_heaven_hell import AntEnv
...
def create_pf_dataset(
    ...
    """
    Create dataset from the Ant-Heaven-Hell POMDP environment.
    ...
    for traj_idx in tqdm(range(num_trajectories), desc="Collecting trajectories"):
        ...
        particle_filter = AntHeavenHellParticleFilter(num_particles)
        ...
        for t in range(timesteps_per_trajectory):
            ...
            true_direction = env.heaven_direction
            ...
            raw_data[traj_key][t_key] = {
                # Ant position (x, y) with a dummy weight
...
class AntHeavenHellParticleFilter:
    """
    Particle filter for the Ant-Heaven-Hell environment.
...
def raw_to_numpy(raw_data):
    ...
    for traj in raw_data.values():
        for timestep in traj.values():
            ...
            key_sorter = lambda x: int(x.split("_")[1])
            for p_key in sorted(timestep.keys(), key=key_sorter):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate a dataset from the Ant-Heaven-Hell POMDP "
    ...
    parser.add_argument(
        "--output_file",
        type=str,
        default="ant_heaven_hell_pf_dataset.npy",
