"""Bit-for-bit parity of a PRETRAINING run between two checkouts (plan section 7, check E-pretrain).
...
def compare(out_a: Path, out_b: Path) -> int:
    ...
    for rel in sorted(set(files_a) & set(files_b)):
        found = (_compare_checkpoint if rel.endswith(".pt") else _compare_json)(files_a[rel], files_b[rel])
        if found:
    ...
    if not any(rel.endswith(".pt") for rel in set(files_a) & set(files_b)):
        problems.append("no checkpoint (.pt) present on both sides -- nothing was compared")
    if problems:
